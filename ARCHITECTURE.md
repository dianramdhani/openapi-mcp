...
- **Hooks**: Husky + lint-staged (pre-commit, pre-push)
...
- **ESLint**: Lints code for quality and style.
- **simple-import-sort**: Automatically sorts imports and exports.
- **Commitizen**: Provides a guided interface for creating conventional commits.
- **Commitlint**: Ensures commit messages follow the conventional format.
- **Husky**: Manages Git hooks to enforce quality checks.
- **lint-staged**: Runs linters on committed files only.

### Git Hooks

- **pre-commit**: Runs `lint-staged` to ensure all committed code is linted.
- **commit-msg**: Runs `commitlint` to validate the commit message format.
- **pre-push**: Runs `yarn build` to ensure the project compiles before pushing.
